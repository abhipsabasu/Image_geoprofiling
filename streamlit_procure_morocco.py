...
def geocode_location(location_text):
    ...
    """
    try:
        # Add "Morocco" to the search query to improve accuracy for Moroccan locations
        ...
        if google_maps_api_key:
                ...
                <script>
                    function initMap() {{
                        const map = new google.maps.Map(document.getElementById("map"), {{
                            zoom: 6,
                            center: {{ lat: 31.6295, lng: -7.9811 }}, // Morocco center
                        ...
                        const cities = [
                            ...
                            {{ lat: 34.2611, lng: -6.5802, name: "Kenitra" }},
                            {{ lat: 35.7595, lng: -5.8339, name: "Tangier" }},
                            {{ lat: 32.2999, lng: -9.2377, name: "Safi" }},
                            {{ lat: 30.4278, lng: -9.5981, name: "Agadir" }},
                            {{ lat: 33.9716, lng: -6.8498, name: "Meknes" }},
                            {{ lat: 34.2251, lng: -4.0076, name: "Fez" }},
                            {{ lat: 30.4278, lng: -9.5981, name: "Oujda" }}
                        ];
            ...
            if hasattr(st.session_state, 'location_text') and st.session_state.location_text:
                ...
                manual_location = st.text_input(
                    "",
                    placeholder="e.g., Marrakech, Casablanca, Rabat, Tangier, Fez",
            ...
            # Show a basic map of Morocco
            map_data = pd.DataFrame({
                'latitude': [31.6295, 33.5731, 34.0209, 35.7595, 30.4278],
                'longitude': [-7.9811, -7.5898, -6.8416, -5.8339, -9.5981]
            })
            st.map(map_data)

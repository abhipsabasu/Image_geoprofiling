...
def geocode_location(location_text):
    ...
    """
    try:
        # Add "Kazakhstan" to the search query to improve accuracy for Kazakhstani locations
        ...
        if google_maps_api_key:
                ...
                <script>
                    function initMap() {{
                        const map = new google.maps.Map(document.getElementById("map"), {{
                            zoom: 6,
                            center: {{ lat: 47.4979, lng: 68.3569 }}, // Kazakhstan center
                            mapTypeId: google.maps.MapTypeId.ROADMAP
                        }});
                        
                        // Add markers for major cities
                        const cities = [
                            {{ lat: 47.4979, lng: 68.3569, name: "Nur-Sultan" }},
                            {{ lat: 47.1511, lng: 67.8524, name: "Almaty" }},
                            {{ lat: 43.2567, lng: 76.9286, name: "Astana" }},
                            {{ lat: 50.0737, lng: 82.9162, name: "Karagandy" }}
                        ];
            ...
            if hasattr(st.session_state, 'location_text') and st.session_state.location_text:
                ...
                manual_location = st.text_input(
                    "",
                    placeholder="e.g., Astana, Almaty, Nur-Sultan",
            ...
            # Show a basic map of Kazakhstan
            map_data = pd.DataFrame({
                'latitude': [47.4979, 47.1511, 43.2567, 50.0737],
                'longitude': [68.3569, 67.8524, 76.9286, 82.9162]
            })
            st.map(map_data)
